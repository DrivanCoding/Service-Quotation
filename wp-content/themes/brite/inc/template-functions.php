...
function brite_page_title( $atts = array() ) {
    $title = '';
    if ( is_404() ) {
        $title = __( 'Page not found', 'brite' );
    } elseif ( is_search() ) {
        $title = sprintf( __( 'Search Results for &#8220;%s&#8221;', 'brite' ), get_search_query() );
    } elseif ( is_home() ) {
        if ( is_front_page() ) {
...
function brite_site_title() {
    $site_title = get_bloginfo( 'name' );
    echo esc_html( $site_title );
}

function brite_post_title( $atts ) {
    $atts = array_merge(
        array(
            'heading_type' => 'h3',
            'classes'      => 'brite-word-wrap'
        ),
        $atts
    );
...
function brite_post_excerpt( $attrs = array() ) {
    ...
    echo '<div class="brite-post-excerpt">' . get_the_excerpt() . '</div>';
}

function brite_post_thumb_placeholder_classes( $atts = array() ) {
...
function brite_post_thumbnail_classes( $atts = array() ) {
...
function brite_post_thumbnail( $atts = array() ) {
...
function brite_post_meta_date_url( $atts = array() ) {
    $id   = get_the_ID();
...
function brite_post_categories( $attrs = array() ) {
    ...
    $html = "";
    if ( $atts['prefix'] !== '' ) {
        $html .= '<span class="d-inline-block categories-prefix">' . brite_esc_html_preserve_spaces( $atts['prefix'] ) . '</span>';
    }
    if ( $categories ) {
        ...
        $html .= sprintf( '<span class="d-inline-block">%s</span>', esc_html__( 'No Category', 'brite' ) );
    }

    echo $html;
}

function brite_esc_html_preserve_spaces( $text ) {
    return esc_html( str_replace( " ", "&nbsp;", $text ) );
}

function brite_post_tags( $attrs = array() ) {
    $atts = shortcode_atts(
        array(
            'prefix' => '',
        ),
    ...
    $html = '';
    if ( $atts['prefix'] !== '' ) {
        $html .= '<span class="d-inline-block tags-prefix">' . brite_esc_html_preserve_spaces( $atts['prefix'] ) . '</span>';
    }
    if ( $tags ) {
        foreach ( $tags as $tag ) {
            $tag_link  = get_tag_link( $tag->term_id );
            $tag_title = sprintf( __( 'Tag: %s', 'brite' ), $tag->name );
        ...
        $html .= sprintf( '<span class="d-inline-block">%s</span>', esc_html__( 'No Tag', 'brite' ) );
    }
...
function brite_get_nav_direction_wp_name( $type ) {
    return $type == "next" ? $type : "previous";
}


function brite_print_navigation_button( $type, $button_text ) {
    $args = array(
        'prev_text'          => '%title',
        'next_text'          => '%title',
        'in_same_term'       => false,
        'excluded_terms'     => '',
        'taxonomy'           => 'category',
        'screen_reader_text' => __( 'Post navigation', 'brite' ),
    );

    $navigation        = '';
    $direction_wp_name = brite_get_nav_direction_wp_name( $type );
...
function brite_post_nav_button( $atts = array() ) {
    $type = $atts['type'];
    ...
    brite_print_navigation_button( $type, $button_text );

}


function brite_button_pagination( $args, $atts ) {
    $type          = $atts['type'];
    $nav_direction = brite_get_nav_direction_wp_name( $type );
...
function brite_numbers_pagination( $args, $atts ) {
...
function brite_render_pagination( $pagination_type, $atts = array(), $args = array() ) {
...
function brite_archive_nav_button( $attrs = array() ) {
    ...
    brite_render_pagination( 'brite_button_pagination', $atts );
}


function brite_archive_pagination() {
    brite_render_pagination( '\brite_numbers_pagination' );
}

function brite_render_page_comments() {
    if ( ! comments_open() ) {
        return;
    }
    ?>
...
function brite_post_comments( $attrs = array() ) {
    ...
    add_filter( 'comments_template', 'brite_post_comments_template' );
    ...
    remove_filter( 'comments_template', 'brite_post_comments_template' );

    echo $content;
}

function brite_post_comment_form() {

}

function brite_widget_area( $atts ) {
...
function brite_post_meta_time_url() {
    return '';
}

function brite_has_multiple_pages() {
    global $wp_query;
...
function brite_output_sidebar_search_form( $form = '' ) {
...
function brite_post_comments_template( $form = '' ) {
    return 'template-parts/blog/comments.php';
}

function brite_theme_print_footer_copyright() {
    ?>
    <div class="h-global-transition-all">
        &copy; <?php echo date( 'Y' ); ?> <?php bloginfo( 'blogname' ); ?>.
        <?php printf( __( 'Built using WordPress and %s', 'brite' ),
            'Brite Theme'
        ); ?> .
    </div>
    <?php
...
function brite_print_offscreen_copyright() {
    echo "&copy; " . date( 'Y' );
}

function brite_the_date( $format = 'F j, Y' ) {
    echo get_the_date( $format );
}

add_filter( 'get_search_form', "brite_output_sidebar_search_form", 100 );


function brite_layout_wrapper( $atts ) {
    $atts = array_merge(
        array(
            "name" => "",
            "slug" => ""
...
function brite_layout_wrapper_output_tags_container( $atts ) {
    if ( has_tag() ) {
...
function brite_layout_wrapper_output_categories_container( $atts ) {
    if ( has_category() ) {
        View::partial( 'layout-wrapper-content', $atts['slug'] );
    }
}
...
function brite_layout_wrapper_output_navigation_container( $atts ) {
    if ( brite_has_multiple_pages() ) {
        View::partial( 'layout-wrapper-content', $atts['slug'] );
    }
}

Hooks::prefixed_add_action( "layout_wrapper_output_tags_container",
    'brite_layout_wrapper_output_tags_container' );

Hooks::prefixed_add_action( "layout_wrapper_output_categories_container",
    'brite_layout_wrapper_output_categories_container' );

Hooks::prefixed_add_action( "layout_wrapper_output_navigation_container",
    'brite_layout_wrapper_output_navigation_container' );
