...
(function ($) {
    ...
    function activateBuilder(callback) {
        ...
        wp.ajax.post('althea_wp_activate_plugin', { slug: althea_wp_builder_status.slug }).done(function () {
            $(window).off('beforeunload.althea-wp-admin-big-notice');
            if (callback) {
                callback();
            } else {
    ...
    function processBuilderInstalationStepts(callback) {
        wp.ajax.post('althea_wp_front_set_predesign', { index: selectedFrontPage });
