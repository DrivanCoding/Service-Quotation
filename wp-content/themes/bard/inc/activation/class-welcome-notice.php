...
class Bard_Welcome_Notice {
	...
	*/
	public function __construct() {
		...
		add_action( 'wp_ajax_bard_dismissed_handler', [$this, 'dismissed_handler'] );
		...
		// Install Plugins
		add_action( 'wp_ajax_bard_install_activate_elementor', [$this, 'install_activate_elementor'] );
		add_action( 'wp_ajax_nopriv_bard_install_activate_elementor', [$this, 'install_activate_elementor'] );
		add_action( 'wp_ajax_bard_install_activate_royal_addons', [$this, 'install_activate_royal_addons'] );
		add_action( 'wp_ajax_nopriv_bard_install_activate_royal_addons', [$this, 'install_activate_royal_addons'] );

		add_action( 'wp_ajax_bard_cancel_elementor_redirect', [$this, 'bard_cancel_elementor_redirect'] );
	}
	...
	*/
	public function install_activate_elementor() {
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error( esc_html__( 'Insufficient permissions to install the plugin.', 'bard' ) );
			wp_die();
		}
		...
		wp_send_json_error( esc_html__( 'Failed to initialize or activate importer plugin.', 'bard' ) );
	...
	*/
	...
	public function bard_cancel_elementor_redirect() {
		exit;
	}
	...
	public function render_notice() {
		...
		if ( 'wpr-addons' !== $screen->parent_base ) {
			...
			if ( ! get_transient( $transient_name ) ) {
				?>
				<div class="bard-notice notice notice-success is-dismissible" data-notice="<?php echo esc_attr( $transient_name ); ?>">
	...
	*/
	public function render_notice_content() {
		...
		if ( 'appearance_page_about-bard' === $screen->id ) {
		...
		<div class="welcome-message" style="<?php echo esc_attr($flex_attr); ?>">
			<h1 style="<?php echo esc_attr($display_attr); ?>"><?php esc_html_e('Welcome to Bard', 'bard'); ?></h1>
			<p>
				<?php esc_html_e('Bard theme now comes with Elementor based sites library with various designs to pick from.', 'bard'); ?>
				<br>
				<?php esc_html_e('Don\'t miss our unique collection of constantly increasing Royal Elementor Templates Kit.', 'bard'); ?>
			</p>
			
			<div class="action-buttons">
				<a href="<?php echo esc_url(admin_url($redirect_url)); ?>" class="button button-primary" data-action="<?php echo esc_attr($action); ?>" data-freemius="<?php echo esc_attr($freemius_passed); ?>"><?php esc_html_e('Get Started with Templates Kit', 'bard'); ?></a>
				
				<a href="<?php echo esc_url(admin_url('themes.php?page=about-bard')); ?>" class="button"  style="<?php echo esc_attr($display_attr); ?>"><?php esc_html_e('Get Started with Bard Blog', 'bard'); ?></a>
			</div>
		</div>
	...
	*/
	public function notice_enqueue_scripts( $page ) {
		...
		<script>
			jQuery(function($) {
				$( document ).on( 'click', '.bard-notice .notice-dismiss', function () {
					jQuery.post( 'ajax_url', {
						action: 'bard_dismissed_handler',
						notice: $( this ).closest( '.bard-notice' ).data( 'notice' ),
					});
					$( '.bard-notice' ).hide();
				} );
			});
	...
	*/
	public function admin_enqueue_scripts( $page ) {
		...
		wp_localize_script( 'welcome-notic-js', 'bard_localize', [
			'ajax_url' => admin_url( 'admin-ajax.php' ),
			'elementor_nonce' => wp_create_nonce( 'nonce' ),
			'royal_addons_nonce' => wp_create_nonce( 'nonce' ),
			'failed_message' => esc_html__( 'Something went wrong, contact support.', 'bard' ),
		] );
